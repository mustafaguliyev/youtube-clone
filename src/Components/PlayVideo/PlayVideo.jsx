...
import moment from "moment"; // vaxt formatlama üçün
...
function PlayVideo() {
  ...
  const fetchVideoData = async () => {
    try {
      ...
      setApiData(data.items?.[0] || null); // undefined olarsa null
    } catch (error) {
  ...
  const fetchOtherData = async () => {
    ...
    await fetch(channelData_url)
      .then((res) => res.json())
      .then((data) => setChannelData(data.items?.[0] || null));
  ...
  return (
    <div className="play-video">
        ...
        <p>
          {apiData
            ? `${value_converter(
              ...
              )} Views • ${moment(apiData.snippet.publishedAt).fromNow(true)} ago`
            : "16K Views • 1 day ago"}
        </p>
      ...
      <div className="vid-description">
        ...
        <p>Subscribe MUtube to watch more tutorials on web development</p>
        <hr />
        <h4>
